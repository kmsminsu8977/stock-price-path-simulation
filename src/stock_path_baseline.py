...
PROJECT = {"analysis_type": "stock_paths", "category": "Stochastic Models & Market Risk", "columns": ["scenario_id", "spot", "annual_mu", "annual_sigma", "horizon_days", "n_steps", "n_paths", "down_barrier", "seed"], "methods": ["GBM 이산화로 전체 경로를 만들고 terminal 가격과 경로 중 최저점을 기록한다.", "하방 barrier 터치 확률과 평균 최대낙폭을 함께 보아 단순 terminal 분석의 한계를 보완한다.", "모든 난수는 고정 seed와 합성 가정으로 생성한다."], "module_name": "stock_path_baseline", "objective": "GBM 경로를 생성하고 terminal 분포, drawdown, barrier touch 확률을 요약한다.", "question": "동일한 기대수익률에서도 변동성 또는 기간이 바뀌면 경로 리스크는 얼마나 커지는가?", "rows": [["ONE_YEAR_BASE", 100, 0.07, 0.2, 252, 63, 2500, 82, 901], ["HIGH_VOL_BARRIER", 100, 0.05, 0.34, 252, 63, 2500, 78, 902], ["SHORT_HORIZON", 100, 0.06, 0.18, 126, 42, 2500, 86, 903]], "sample_name": "gbm_path_assumptions.csv", "title_en": "Stock Price Path Simulation", "title_ko": "주가 경로 시뮬레이션"}
